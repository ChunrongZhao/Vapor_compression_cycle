...
from VCS_Codes.Components.VCS_components         import VCS_condenser_Finned_Tube_3Zones, Battery_Wavychannel_Evaporator_2Zones, \
    VCS_condenser_Finned_Tube_3Zones_herringbone, VCS_evaporator_Finned_Tube_2Zones, Battery_Wavychannel_SinglePhaseCooler
...
from ACHP_codes.Correlations.FinStructure_Correlations import HerringboneFins_condenser, HerringboneFins_evaporator
...
class Cycle_Performance_BatteryAir_herringbone:
    ...
    def __init__(self, SysParams, dt, Q_bat, T_bat, m_dot_air, T_air_inlet, T_coolant_in_5, eta_fin_cond, fin_ratio_cond, eta_fin_evap, fin_ratio_evap, h_c_evap, m_dot_c, h_air_cond, air, refrigerant):
        ...
        # ------------------added at 20/11/2024 ---------------------------
        self.eta_fin_air                = eta_fin_cond
        self.fin_ratio_air              = fin_ratio_cond
        self.eta_fin_c                  = eta_fin_evap
        self.fin_ratio_c                = fin_ratio_evap
        self.T_coolant_in_5             = T_coolant_in_5

        self.dec                        = 10    # decimal for some PropsSI functions
        self.CD_HTC_air                 = h_air_cond   # W/m2K, heat transfer coefficient of Condenser: air side
        self.HTC_coolant                = h_c_evap   # W/m2K, heat transfer coefficient of Evaporator: liquid coolant side
    ...
    def March_Compressor_Inlet_and_Outlet_Pressures(self, P_guess):
        ...
        if P_guess[0] < 0 or P_guess[1] < 0:
            ...
            condenser                   = VCS_condenser_Finned_Tube_3Zones_herringbone(self.refrigerant, self.heat_transfer_fluid_cond, self.T_air_inlet,
                                          self.m_dot_refrigerant, self.m_dot_air, P_guess[1], self.T_2, self.subcooling, self.CD_HTC_air, self.eta_fin_air, self.fin_ratio_air)
            ...
            evaporator                   = VCS_evaporator_Finned_Tube_2Zones(self.refrigerant, self.T_coolant_in_5, self.m_dot_refrigerant, self.m_dot_c,
                                                                             round(P_guess[0], 2), self.h_4, self.superheating, self.HTC_coolant, self.eta_fin_c, self.fin_ratio_c)
            """refrigerant, T_coolant_inlet, m_dot_ref, m_dot_coolant, P_i, h_i, SH_degree, HTC_coolant"""
            ...
            # ---------------------------------------------------------
            self.T_coolant_out_6            = evaporator_results.T_coolant_out
            # ---------------------------------------------------------
            # Coolant Properties
            rho_c               = 1075
            k_c                 = 0.387
            ...
            # ---------------------------------------------------------------
            # Point (6)7 --> 8(5)
            BatteryCooler = Battery_Wavychannel_SinglePhaseCooler(self.dt, self.Q_bat, self.T_bat, self.m_dot_c, self.T_coolant_out_6)
            battery_results                 = BatteryCooler.thermodynamics_calculation()
            # ---------------------------------------------------------------
            self.T_bat_updated                   = battery_results.T_bat_updated
            self.T_o_8                           = battery_results.T_o
            self.Q_convec                        = battery_results.Q_convec
            self.eff_WavyChan                    = battery_results.eff_WavyChan
            self.P_pump_CHAN_coolant             = battery_results.P_pump_CHAN_coolant
            # ---------------------------------------------------------------
...
def HerringboneFinCal(V_dot_ha, T_air_inlet, p=101325, RH=0.51):
        Fins        = AttributeDict()
        Fins.Tubes.N_Tubes_per_bank     = 50                    # number of tubes per bank=row
        ...
        A, eta_o, h_a, DeltaP_air, m_dot_da, m_dot_ha, fin_ratio, P_FAN_COND_air = HerringboneFins_condenser(Fins)
        ...
        return m_dot_air_cal, A_fin, eta_fin, HTC_air_cal, fin_ratio, P_FAN_COND_air


# todo added at 18/11/2024
def HerringboneFinCal_evap(m_dot_coolant):
        Fins        = AttributeDict()
        ...
        A_evap, eta_o_c_evap, h_c_evap, DeltaP_c_evap, fin_ratio_evap, P_pump_EVAP_coolant = HerringboneFins_evaporator(Fins, m_dot_coolant)

        return A_evap, eta_o_c_evap, h_c_evap, DeltaP_c_evap, fin_ratio_evap, P_pump_EVAP_coolant
...
# -----HTC ---------------
# 2->3
def HTC_cond_2ph_comparison(Tsat, m_dot_r, Number_of_tubes=5, D=0.01, HTC=4274):
...
def HTC_cond_vap_comparison(Tsat, T_in_r, m_dot_r, Number_of_tubes=5, D=0.01, HTC=1291):
    AS                          = CP.AbstractState('HEOS', 'R134a')
    # --------------------------------------------------------
...
def HTC_cond_liq_comparison(Tsat, T_out_r, m_dot_r, Number_of_tubes=5, D=0.01, HTC=1414):
    AS                          = CP.AbstractState('HEOS', 'R134a')
    # --------------------------------------------------------
    m_dot               = m_dot_r / Number_of_tubes
...
# 4->1
def HTC_evap_2ph_comparison(Tsat, m_dot_r, x_in, L_2ph, Number_of_tubes=5, D=0.01, HTC=3706):
    ...
    h_fg                        = h_v - h_l                 # [J/kg]
    Q_targe                     = m_dot_r * (1.0 - x_in) * h_fg   # [W]
    Ar                          = Number_of_tubes * np.pi * D * L_2ph
    ...
    * T_dew : Dew point temperature of refrigerant [K]
    ...
    for i in range(len(x)):
        h[i] = ShahEvaporation_Average(x[i], x[i], AS, G, D, p_sat_r, q_flux, Tsat, Tsat)
...
def HTC_evap_vap_comparison(Tsat, T_out_r, m_dot_r, Number_of_tubes=5, D=0.01, HTC=1076):
    AS                          = CP.AbstractState('HEOS', 'R134a')
    # --------------------------------------------------------
    m_dot                               = m_dot_r / Number_of_tubes
...
def plot_BTMS_results1(BTMS_results, DT_amb, width=10, height=9):
    # ---------------------------------------------------------------------
    #   set the font style
    # ---------------------------------------------------------------------
    ...
    markers = ['o', '^', 's', 'p', 'v', '*', 'x', 'H', 'X', 'P', 'D']

    # ---------------------------------------------------------------------
    #   load results
    # ---------------------------------------------------------------------
    blob_size                           = 100 /2                # for solutions in plot
    x_label                             = 'Time (min)'
    y1_label                            = '$T_{bat}$ ($^\circ$C)'
    y2_label                            = '$L_{2ph}$ fraction (%)'
    y3_label                            = 'COP (-)'
    y4_label                            = '$W_{comp}$ (kW)'
    y5_label                            = '$Q_{bat}$ (kW)'
    y6_label                            = '$Q_{conv}$ (kW)'
    ...
    plt.subplot(3, 2, 1)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T_bat_liq'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$T_{bat,vcs}$')
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T_bat_air'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[1], linewidths=line_width, label='$T_{bat,air}$')
    ...
    plt.subplot(3, 2, 2)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['L_2ph_evap'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='evaporator')
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['L_2ph_cond'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[1], linewidths=line_width, label='condenser')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y2_label)
    ...
    plt.subplot(3, 2, 3)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['COP_standalone'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='standalone')
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['COP_overall'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[1], linewidths=line_width, label='overall')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y3_label)
    ...
    plt.subplot(3, 2, 4)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['Power'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y4_label)
    ...
    plt.subplot(3, 2, 5)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['Q_bat'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    ...
    plt.subplot(3, 2, 6)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['Q_conv'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$Q_{chan}$')
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['Q_evap'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[1], linewidths=line_width, label='$Q_{evap}$')
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['Q_cond'][:-10], marker=markers[2], alpha=alpha, s=blob_size, c=colors[2], linewidths=line_width, label='$Q_{cond}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'fig1_deltaT={DT_amb}.png', dpi=image_resolution, format='png')


def plot_BTMS_results2(BTMS_results, DT_amb, width=10, height=9):
    # ---------------------------------------------------------------------
    #   set the font style
    # ---------------------------------------------------------------------
    # Let's set the font to the one we want
    ...
    y8_label                            = '$m_{VCC}$ (kg)'
    y9_label                            = '$P_1$ (kPa)'
    y10_label                            = '$T_{sat,1}$ ($^\circ$C)'
    y11_label                            = '$P_2$ (kPa)'
    y12_label                            = '$T_{sat,2}$ ($^\circ$C)'
    y13_label                            = '$T_{c,in}$ ($^\circ$C)'
    y14_label                            = '$T_{c,out}$ ($^\circ$C)'
    # ---------------------------------------------------------------------
    #   plot results
    # ---------------------------------------------------------------------
    fig1 = plt.figure(None)
    ...
    plt.subplot(4, 2, 1)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['m_dot_ref'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y7_label)
    ...
    plt.subplot(4, 2, 2)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['m_VCC'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y8_label)
    ...
    plt.subplot(4, 2, 3)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['P1'][:-10], marker=markers[2], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y9_label)
    ...
    plt.subplot(4, 2, 4)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T1_sat'][:-10], marker=markers[3], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    ...
    plt.subplot(4, 2, 5)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['P2'][:-10], marker=markers[4], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    ...
    plt.subplot(4, 2, 6)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T2_sat'][:-10], marker=markers[5], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    ...
    plt.subplot(4, 2, 7)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T_coolant_in'][:-10], marker=markers[6], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    plt.ylabel(y13_label)

    plt.subplot(4, 2, 8)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['T_coolant_out'][:-10], marker=markers[7], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    plt.ylabel(y14_label)

    plt.tight_layout()
    # plt.show()
    plt.savefig(f'fig2_deltaT={DT_amb}.png', dpi=image_resolution, format='png')


def plot_BTMS_results3(BTMS_results, DT_amb, width=10, height=9):
    # ---------------------------------------------------------------------
    #   set the font style
    # ---------------------------------------------------------------------
    # Let's set the font to the one we want
    ...
    x_label                             = 'Time (min)'
    y0_label                            = '$h$ (W/m$^2$K)'
    y1_label                            = '$h_{coolant,cond}$ (W/m$^2$K)'
    ...
    plt.subplot(4, 3, 1)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_air'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{air,cond,out}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 2)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_coolant'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{coolant,evap,out}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 3)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_2ph_cond'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{cond,2ph}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 4)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_liq_cond'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{cond,liq}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 5)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_vap_cond'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{cond,vap}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 6)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_2ph_evap'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{evap,2ph}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 7)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['HTC_vap_evap'][:-10], marker=markers[0], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width, label='$h_{evap,liq}$')
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y0_label)
    ...
    plt.subplot(4, 3, 8)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['err_HTC_liq_cond'][:-10], marker=markers[1], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y8_label)
    ...
    plt.subplot(4, 3, 9)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['err_HTC_2ph_cond'][:-10], marker=markers[2], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y9_label)
    ...
    plt.subplot(4, 3, 10)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['err_HTC_vap_cond'][:-10], marker=markers[3], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.ylabel(y10_label)
    ...
    plt.subplot(4, 3, 11)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['err_HTC_2ph_evap'][:-10], marker=markers[4], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    ...
    plt.subplot(4, 3, 12)
    plt.scatter(BTMS_results['mission_time'][:-10], BTMS_results['err_HTC_vap_evap'][:-10], marker=markers[5], alpha=alpha, s=blob_size, c=colors[0], linewidths=line_width)
    plt.grid(visible=True, which='major', linestyle='-', linewidth=0.75, color=colors[0])
    plt.minorticks_on()
    plt.grid(visible=True, which='minor', linestyle='--', linewidth=0.15, color=colors[0])
    plt.xlabel(x_label)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'fig3_deltaT={DT_amb}.png', dpi=image_resolution, format='png')


# ---------------------------------------------------------------------
#   Main
# ---------------------------------------------------------------------
def main_herringbone_pre_calculations(DT_amb, V_dot_ha, m_dot_coolant):
    ...
    T_coolant_in_5                  = BatInputs['T_b_air'][0] + DT_amb

    VCS_results = {'mission_time': [], 'T_bat_liq': [], 'T_bat_air': [], 'COP_standalone': [], 'COP_overall': [], 'Power': [], 'L_2ph_evap': [], 'L_2ph_cond': [], 'Q_bat': [],
                   'Q_conv': [], 'Q_evap': [], 'Q_cond': [], 'm_dot_ref': [], 'm_VCC': [], 'P1': [], 'T1_sat': [], 'P2': [], 'T2_sat': [],
                   'HTC_2ph_cond': [], 'HTC_liq_cond': [], 'HTC_vap_cond': [], 'HTC_2ph_evap': [], 'HTC_vap_evap': [], 'HTC_air': [], 'HTC_coolant': [],
    ...
    # airside of the condenser
    m_dot_air_cal_cond, A_fin_cond, eta_fin_cond, h_air_cond, fin_ratio_cond, P_FAN_COND_air = HerringboneFinCal(V_dot_ha, T_air, p=101325, RH=0.51)
    m_dot_air   = m_dot_air_cal_cond

    # coolant side of the evaporator
    A_evap, eta_o_c_evap, h_c_evap, DeltaP_c_evap, fin_ratio_evap, P_pump_EVAP_coolant = HerringboneFinCal_evap(m_dot_coolant)
    eta_fin_evap    = eta_o_c_evap

    HP        = Cycle_Performance_BatteryAir_herringbone(SysParams, dt, Q_bat, T_bat, m_dot_air, T_air, T_coolant_in_5, eta_fin_cond, fin_ratio_cond,
                                                         eta_fin_evap, fin_ratio_evap, h_c_evap, m_dot_coolant, h_air_cond, 'air', 'R134a')
    HP.Cycle_Solver()
    # HP.PostProcessing()
    ...
    # condenser, airside
    h_air_cond, err_h_air_cond = HTC_cond_airside_finned_tube(h_air_cond)

    # print('h_2ph_evap:  ', h_2ph_evap, ';   h_vap_evap: ', h_vap_evap)
    # print('h_2ph_cond:  ', h_2ph_cond, ';   h_vap_cond: ', h_vap_cond, ';   h_liq_cond: ', h_liq_cond)

    return h_2ph_evap, h_vap_evap, h_2ph_cond, h_vap_cond, h_liq_cond


def main_herringbone(DT_amb, V_dot_ha, m_dot_coolant, h_2ph_evap1, h_vap_evap1, h_2ph_cond1, h_vap_cond1, h_liq_cond1):
    # Parameters of a generic HP to be used in the VC model
    # ------------------------------------------------------------------------------
    Comp        = {'Comp: rpm': 3.6e3*2,      'Comp: eta_C_iso': 0.63,    'Comp: Disp': 4.25e-5*1, 'Comp: eta_C_vol': 0.95, 'Comp: eta_C_mec': 0.88}
    # Evap        = {'Evap: TubeL': 10,       'Evap: TubeN': 5,           "Evap: TubeID": 0.011, 'Evap: FinEff': 0.75,    'Evap: FinRatio': 7.2, 'Evap: Pump': 220}
    ...
    VCS_results = {'mission_time': [], 'T_bat_liq': [], 'T_bat_air': [], 'COP_standalone': [], 'COP_overall': [], 'Power': [], 'L_2ph_evap': [], 'L_2ph_cond': [], 'Q_bat': [],
                   'Q_conv': [], 'Q_evap': [], 'Q_cond': [], 'm_dot_ref': [], 'm_VCC': [], 'P1': [], 'T1_sat': [], 'P2': [], 'T2_sat': [], 'T_coolant_in': [], 'T_coolant_out': [],
                   'HTC_2ph_cond': [], 'HTC_liq_cond': [], 'HTC_vap_cond': [], 'HTC_2ph_evap': [], 'HTC_vap_evap': [], 'HTC_air': [], 'HTC_coolant': [],
                   'err_HTC_2ph_cond': [], 'err_HTC_liq_cond': [], 'err_HTC_vap_cond': [], 'err_HTC_2ph_evap': [], 'err_HTC_vap_evap': [], 'err_HTC_air': []}
    # ------------------------------------------------------
    for i in range(1, len(BatInputs['T_amb'])):
        ...
        # ----------
        # 2-> 3
        h_vap_cond, err_h_vap_cond = HTC_cond_vap_comparison(HP.state_3[2] + HP.subcooling, HP.state_2[2], HP.m_dot_refrigerant, HTC=h_vap_cond1)
        h_2ph_cond, err_h_2ph_cond = HTC_cond_2ph_comparison(HP.state_3[2] + HP.subcooling, HP.m_dot_refrigerant, HTC=h_2ph_cond1)
        h_liq_cond, err_h_liq_cond = HTC_cond_liq_comparison(HP.state_3[2] + HP.subcooling, HP.state_3[2], HP.m_dot_refrigerant, HTC=h_liq_cond1)
        # 4->1
        h_2ph_evap, err_h_2ph_evap = HTC_evap_2ph_comparison(HP.state_1[2] - HP.superheating, HP.m_dot_refrigerant, HP.state_4[4], HP.evap_L_1, HTC=h_2ph_evap1)
        h_vap_evap, err_h_vap_evap = HTC_evap_vap_comparison(HP.state_1[2] - HP.superheating, HP.state_1[2], HP.m_dot_refrigerant, HTC=h_vap_evap1)
        # condenser, airside
        h_air_cond, err_h_air_cond = HTC_cond_airside_finned_tube(h_air_cond)

        print('h_2ph_evap:  ', h_2ph_evap, ';   h_vap_evap: ', h_vap_evap)
        print('h_2ph_cond:  ', h_2ph_cond, ';   h_vap_cond: ', h_vap_cond, ';   h_liq_cond: ', h_liq_cond)
        # ---------- variable update 20/11/2024 --------------------
        T_bat           = HP.T_bat_updated
        T_coolant_in_5  = HP.T_o_8
        # ---------------------------------------------------
        # print('percentage of evaporator length: ', HP.evap_L_1 / HP.evap_L_tot * 100, ' %')
        # ---------------------------------------------------------
        VCS_results['T_bat_liq'].append(T_bat - 273.15)
        VCS_results['T_bat_air'].append(T_bat_air - 273.15)
        VCS_results['T_coolant_in'].append(HP.T_coolant_in_5 - 273.15)      # For evaporator inlet, corresponding to wavychannel outlet
        VCS_results['T_coolant_out'].append(HP.T_coolant_out_6 - 273.15)
        VCS_results['L_2ph_evap'].append(HP.evap_L_1 / HP.evap_L_tot * 100)
        VCS_results['L_2ph_cond'].append(HP.cond_L_2 / HP.cond_L_tot * 100)
        ...
        VCS_results['Q_conv'].append(HP.Q_convec/1e3)
        VCS_results['Q_evap'].append(HP.Q_evaporator/1e3)
        VCS_results['Q_cond'].append(HP.Q_condenser/1e3)
        # ---------------------------COP updates 20/11/2024-----------------------------------
        COP_standalone      = HP.Q_evaporator / ((HP.W_c / HP.eta_C_mec) + P_FAN_COND_air)
        COP_overall         = HP.Q_bat / ((HP.W_c / HP.eta_C_mec) + P_FAN_COND_air + P_pump_EVAP_coolant + HP.P_pump_CHAN_coolant)
        VCS_results['COP_standalone'].append(COP_standalone)
        VCS_results['COP_overall'].append(COP_overall)
        # -------------------------------------------------------------------------------------
        ...
        VCS_results['T1_sat'].append(HP.state_1[2] - HP.superheating - 273.15)
        VCS_results['P2'].append(HP.state_3[0]/1e3)
        VCS_results['T2_sat'].append(HP.state_3[2] + HP.subcooling - 273.15)

        # -----29/07/2024 ----------------------------------------------------
        VCS_results['HTC_air'].append(h_air_cond)
        VCS_results['HTC_coolant'].append(h_c_evap)
    ...
    plot_BTMS_results1(VCS_results, DT_amb)
    plot_BTMS_results2(VCS_results, DT_amb)
    plot_BTMS_results3(VCS_results, DT_amb)


def cases_amb_temp(V_dot_ha, m_dot_coolant):
    for DT_amb in [0, 5, 10, 15, 20, 25, 30, 35]:
        h_2ph_evap1, h_vap_evap1, h_2ph_cond1, h_vap_cond1, h_liq_cond1 = main_herringbone_pre_calculations(DT_amb, V_dot_ha, m_dot_coolant)
        main_herringbone(DT_amb, V_dot_ha, m_dot_coolant, h_2ph_evap1, h_vap_evap1, h_2ph_cond1, h_vap_cond1, h_liq_cond1)
...
if __name__ == '__main__':
    ...
    V_dot_ha                        = 1.7934 * 0.2
    m_dot_coolant                   = 3.
    cases_amb_temp(V_dot_ha, m_dot_coolant)
