...
def MultiLouveredMicroFins(Inputs):
    ...
    N_tubes                         = Inputs.Tubes.N_Tubes              # Number of tubes
    N_bank                          = Inputs.Tubes.N_bank               # Number of banks
    L3                              = Inputs.Tubes.L_tube               # length of a single tube
    ...
    V_dot_ha                        = Inputs.Air.V_dot_ha               # Air volume flow rate, m^3/s
    p                               = Inputs.Air.p                      # Air pressure, Pa

    if hasattr(Inputs.Air, 'T_mean'):
        Temp                        = Inputs.Air.T_mean
    else:
        Temp                        = Inputs.Air.T_db

    if hasattr(Inputs.Air, 'RH_mean'):
    ...
    # Air passages
    # sometime there are no tubes on the edges of HX, so >>> Npg = N_tubes_bank + 1, otherwise Npg = N_tubes_bank - 1
    Npg                             = N_tubes + 1
    ...
    Ap                              = (2 * (L2 - Ht) + pi * Ht) * L3 * N_tubes - 2 * delta * L2 * nf
    ...
    h_da                            = HAPropsSI('H', 'T', Inputs.Air.T_db, 'P', p, 'W', W)
    ...
    Pr                              = cp_ha * mu_ha / k_ha                      # Prandtl's number
    ...
    j   = pow(Re_lp, -0.487) * pow((Lalpha / 90.0), 0.257) * pow(pf / lp, -0.13) * pow(b / lp, -0.29) * pow(Lf / lp, -0.235) * pow(Llouv / lp, 0.68) \
          * pow(pt / lp, -0.279) * pow(delta / lp, -0.05)
    ...
    # cs_cp is the correction for heat/mass transfer
    mf                              = sqrt(2 * h_a * cs_cp / (k_fin * delta) * (1 + delta / Lf))
    # characteristic length (Based on Kim & Bullard 2002 paper)
    Ls                              = sf / 2 - delta
    # Finned surface efficiency
    # Can be included for wet surface >>> *cos(0.1 * m * Ls)
    eta_f                           = tanh(mf * Ls) / (mf * Ls)
...
if __name__ == '__main__':
    ...
    LouversFinsTubes.Tubes.N_Tubes                  = 61.354                # Number of tubes
    LouversFinsTubes.Tubes.N_bank                   = 1                     # Number of banks (set to 1 for now!)
    LouversFinsTubes.Tubes.N_pass                   = 3                     # Number of passes (per bank)
    LouversFinsTubes.Tubes.N_ports                  = 1                     # Number of rectangular ports
    LouversFinsTubes.Tubes.L_tube                   = 0.30213               # length of a single tube
    ...
    LouversFinsTubes.Tubes.k_w                      = 117                   # wall thermal conductivity
    ...
    LouversFinsTubes.Air.V_dot_ha                   = 1.05                  # Air volume flow rate in m^3/s
    LouversFinsTubes.Air.T_db                       = 298                   # Air inlet temperature, K
