...
class PlotsClass():
    ...
    def TSOverlay(self, Cycle, **kwargs):
        ...
        if Cycle.Compressor.T_out_r > T_critical and Cycle.Compressor.p_out_r > p_critical:
            ...
            self.axes.plot(s_cond_1, T_cond_range1, 'g-.', lw=1, alpha=0.8) # liquid range
            self.axes.plot(s_cond_2, T_cond_range2, 'c-.', lw=1, alpha=0.8) # vapor range
            self.axes.plot([s_dew_cond / 1000, s_bubble_cond / 1000], [T_dew_cond, T_bubble_cond], 'm-.', lw=5, alpha=0.3)  # mixture range

        # Critical isothermal
        self.axes.plot(s_lim, T_lim, 'y-.', lw=3, alpha=0.8)    # isothermal line at p = p_critical

        # Ts saturated lines
        self.axes.plot(s_sat_L, T_sat, 'k', lw=15, alpha=0.5)   # liquid line at temperature [T_min, T_critical]
        self.axes.plot(s_sat_V, T_sat, 'k', lw=15, alpha=0.5)   # vapor line at temperature [T_min, T_critical]

        # Isolines evaporating temperature
        self.axes.plot(s_evap_1, T_evap_range1, 'b-.', lw=3, alpha=0.3) # liquid range
        self.axes.plot(s_evap_2, T_evap_range2, 'g-.', lw=3, alpha=0.3) # vapor range
        self.axes.plot([s_dew_evap / 1000, s_bubble_evap / 1000], [T_dew_evap, T_bubble_evap], 'm-.', lw=3, alpha=0.3)  # mixture range
        ...
        if Cycle.Compressor.T_out_r > T_critical  and Cycle.Compressor.p_out_r > p_critical:
            ...
            # Axes limits
            self.axes.set_xlim(0.7 * Cycle.Condenser.s_out_r / 1000, 1.2 * Cycle.Compressor.s_out_r / 1000) # [1.2*0.7, 1.855*1.2] [point-3, point-2]
            self.axes.set_ylim(0.9 * Cycle.Evaporator.T_out_r, 1.1 * Cycle.Compressor.T_out_r)  # [286.8*0.9, 348.6*1.1] [point-1, point-2]

        # -------------------------------------------------
        # compressor points and lines
        if Cycle.CycleType == '1INJ_Econ' or Cycle.CycleType == '1INJ_FlashTank':
            ...
            s_comp = np.r_[Cycle.Compressor.s_in_r / 1000., Cycle.Compressor.s_out_r / 1000.]   # [1.823, 1.855]
            T_comp = np.r_[Cycle.Compressor.T_in_r, Cycle.Compressor.T_out_r]   # [286.8, 348.6]
            self.axes.plot(s_comp, T_comp, 'y', lw=2)
            self.axes.plot(s_comp[0], T_comp[0], 'ko', mfc='y') # point-1
            self.axes.plot(s_comp[1], T_comp[1], 'ko', mfc='m') # point-2
                                        # mfc= markerfacecolor; 'w' = white
                                        # https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.plot.html
            self.axes.text(s_comp[0], T_comp[0], ' 1', ha='left', va='top') # ha = horizontalalignment;
                                        # https://matplotlib.org/stable/users/explain/text/text_props.html
            self.axes.text(s_comp[1], T_comp[1], ' 2', ha='left', va='bottom') # va = verticalalignment
        ...
        if (Cycle.CycleType == 'Secondary' and Cycle.Mode == 'AC') or Cycle.CycleType == 'DX':
            ...
            T_cond = np.r_[Cycle.Condenser.T_in_r, T_dew_cond, T_bubble_cond, Cycle.Condenser.T_out_r] # [348.6, 317.4, 317.3, 310.3]
            self.axes.plot(s_cond, T_cond, 'k', lw=4)   # lw = linewidth
            self.axes.plot(s_cond[3], T_cond[3], 'ko', mfc='w')
            self.axes.text(s_cond[3], T_cond[3], '3$\quad\quad$', ha='right', va='bottom')

            self.axes.plot([s_cond[3], s_cond[0]], [Cycle.Condenser.T_in_a, Cycle.Condenser.T_out_a], 'r-') # air temperature cross condenser
        ...
        if Cycle.CycleType == "Secondary":
            ...
            self.axes.plot(s_evap, T_evap, 'm', lw=4)
            self.axes.plot(s_evap[0], T_evap[0], 'ko', mfc='m')
            ...
            self.axes.plot(s_XV, T_XV, 'y', lw=5)
...
class SL_PlotsClass():
    ...
    def TSOverlay(self, Cycle, **kwargs):
        ...
        if Cycle.Compressor.T_out_r > T_critical  and Cycle.Compressor.p_out_r > p_critical:
            ...
            # Axes limits
            self.axes.set_xlim(0.7 * Cycle.Condenser.s_out_r / 1000, 1.2 * Cycle.Compressor.s_out_r / 1000) # [1.2*0.7, 1.855*1.2] [point-3, point-2]
            self.axes.set_ylim(0.9 * Cycle.CoolingCoil.T_out_g, 1.1 * Cycle.Compressor.T_out_r)  # [286.8*0.9, 348.6*1.1] [point-1, point-2]
        ...
        if Cycle.CycleType == "Secondary":
            if Cycle.IHXType == 'Coaxial':
                ...
                if Cycle.Mode == 'AC':
                    ...
                    sV              = PropsSI('S', 'T', Cycle.T_dew_evap, 'Q', 1, Ref) / 1000
                    s_IHX           = np.r_[IHX.s_in_c / 1000., sV, IHX.s_out_c / 1000.]
                    T_IHX           = np.r_[IHX.T_in_c, Cycle.T_dew_evap, IHX.T_out_c]  # [278.08, 278.16, 283.16] [point 4, sat., 1]

                    s_XV            = np.r_[IHX.s_in_c / 1000., Cycle.Condenser.s_out_r / 1000.]
                    T_XV            = np.r_[IHX.T_in_c, Cycle.Condenser.T_out_r]    # [278.08, 309.89] [point 4, 3]

                    T_IHXg          = np.r_[IHX.T_out_h, IHX.T_in_h]    # [280.6, 287.17] [point 9,8]
                    s_IHXg          = np.r_[s_IHX[0], s_IHX[len(s_IHX)-1]]

                    self.axes.text(s_IHX[0], T_IHX[0], '4$\quad\quad$', ha='right', va='top')
                    self.axes.plot(s_IHX[0], T_IHX[0], 'ko', mfc='w')

                    # newly added at 08/12/2023
                    AS_SLF          = CP.AbstractState(Cycle.Backend_SLF, Cycle.SecLoopFluid)
                    # point 5 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, Cycle.CoolingCoil.p_in_g, Cycle.CoolingCoil.T_in_g)
                    s_CC_in         = AS_SLF.smass() / 1000

                    # self.axes.text(s_IHX[0], Cycle.CoolingCoil.T_in_g, '5$\quad\quad$', ha='right', va='top')
                    # self.axes.plot(s_IHX[0], Cycle.CoolingCoil.T_in_g, 'ko', mfc='b')

                    # point 6 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, Cycle.CoolingCoil.p_in_g, Cycle.CoolingCoil.T_out_g)
                    s_CC_out         = AS_SLF.smass() / 1000

                    # self.axes.text(s_IHX[2], Cycle.CoolingCoil.T_out_g, '6$\quad\quad$', ha='right', va='top')
                    # self.axes.plot(s_IHX[2], Cycle.CoolingCoil.T_out_g, 'ko', mfc='r')

                    # point 8 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, IHX.p_in_h, IHX.T_in_h)
                    s_IHX_in         = AS_SLF.smass() / 1000

                    self.axes.text(s_IHX[2], IHX.T_in_h, '8$\quad\quad$', ha='right', va='top')
                    self.axes.plot(s_IHX[2], IHX.T_in_h, 'ko', mfc='r')

                    # point 9 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, IHX.p_in_h, IHX.T_out_h)
                    s_IHX_out         = AS_SLF.smass() / 1000

                    self.axes.text(s_IHX[0], IHX.T_out_h, '9$\quad\quad$', ha='right', va='top')
                    self.axes.plot(s_IHX[0], IHX.T_out_h, 'ko', mfc='b')

                    # ------------------------------------------------------------
                    # line 9-5
                    # T_Line_Supply   = np.r_[IHX.T_out_h, Cycle.CoolingCoil.T_in_g]    # [point 9,5]
                    # s_Line_Supply   = np.r_[s_IHX_out, s_CC_in]
                    T_Line_Supply   = np.r_[IHX.T_out_h, Cycle.CoolingCoil.T_in_g]    # [point 9,5]
                    s_Line_Supply   = np.r_[s_IHX[0], s_IHX[0]]
                    # line 5-6
                    # T_CC            = np.r_[Cycle.CoolingCoil.T_out_g, Cycle.CoolingCoil.T_in_g]    # [point 6,5]
                    # s_CC            = np.r_[s_CC_out, s_CC_in]
                    T_CC            = np.r_[Cycle.CoolingCoil.T_out_g, Cycle.CoolingCoil.T_in_g]    # [point 6,5]
                    s_CC            = np.r_[s_IHX[2], s_IHX[0]]
                    # line 6-8
                    # T_Line_Return   = np.r_[Cycle.CoolingCoil.T_out_g, IHX.T_in_h]    # [point 6,8]
                    # s_Line_Return   = np.r_[s_CC_out, s_IHX_in]
                    T_Line_Return   = np.r_[Cycle.CoolingCoil.T_out_g, IHX.T_in_h]    # [point 6,8]
                    s_Line_Return   = np.r_[s_IHX[0], s_IHX[0]]
                    # line 9-8
                    # T_IHX_g_new   = np.r_[IHX.T_out_h, IHX.T_in_h]    # [point 9,8]
                    # s_IHX_g_new   = np.r_[s_IHX_out, s_IHX_in]
                    T_IHX_g_new   = np.r_[IHX.T_out_h, IHX.T_in_h]    # [point 9,8]
                    s_IHX_g_new   = np.r_[s_IHX[0], s_IHX[2]]
            ...
            self.axes.plot(s_IHX, T_IHX, 'k', lw=2) # line 4-1

            self.axes.plot(s_XV, T_XV, 'k', lw=2) # line 3-4
            self.axes.plot([s_IHX[2], s_IHX[0]], [Cycle.CoolingCoil.T_in_a, Cycle.CoolingCoil.T_out_a], 'b-')
            self.axes.text(0.5 * s_IHX[2] + 0.5 * s_IHX[0], Cycle.CoolingCoil.T_in_a, 'Indoor Air', backgroundcolor='w', ha='center', va='center')
            self.axes.text(0.5 * s_IHXg[0] + 0.5 * s_IHXg[1], np.mean(T_IHXg), 'Glycol IHX', backgroundcolor='w', ha='center', va='center')
            self.axes.plot(s_IHXg, T_IHXg, 'g-.') # line 8-9
            # newly added at 08/12/2023
            # self.axes.plot(s_IHX_g_new, T_IHX_g_new, 'b-.') # line 8-9
            # self.axes.plot(s_Line_Supply, T_Line_Supply, 'b-.')
            # self.axes.plot(s_CC, T_CC, 'b-.')
            # self.axes.plot(s_Line_Return, T_Line_Return, 'b-.')
            print('T_8 = ', IHX.T_in_h, ' [K]; T_9 = ', IHX.T_out_h, ' [K]; T_5 = ', Cycle.CoolingCoil.T_in_g, ' [K]; T_6 = ', Cycle.CoolingCoil.T_out_g, '[K]')
...
class BTMS_SL_PlotsClass():
    ...
    def TSOverlay(self, Cycle, **kwargs):
        ...
        if Cycle.Compressor.T_out_r > T_critical  and Cycle.Compressor.p_out_r > p_critical:
            ...
            # Axes limits
            self.axes.set_xlim(0.7 * Cycle.Condenser.s_out_r / 1000, 1.2 * Cycle.Compressor.s_out_r / 1000) # [1.2*0.7, 1.855*1.2] [point-3, point-2]
            self.axes.set_ylim(0.9 * Cycle.WavyChannel.T_out_g, 1.1 * Cycle.Compressor.T_out_r)  # [286.8*0.9, 348.6*1.1] [point-1, point-2]
        ...
        if Cycle.CycleType == "Secondary":
            if Cycle.IHXType == 'Coaxial':
                ...
                if Cycle.Mode == 'AC':
                    ...
                    AS_SLF          = CP.AbstractState(Cycle.Backend_SLF, Cycle.SecLoopFluid)
                    # point 5 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, Cycle.WavyChannel.p_in_g, Cycle.WavyChannel.T_in_g)
                    s_CC_in         = AS_SLF.smass() / 1000
                    ...
                    # point 6 ----------------------------------------------------------------------
                    AS_SLF.update(CP.PT_INPUTS, Cycle.WavyChannel.p_in_g, Cycle.WavyChannel.T_out_g)
                    s_CC_out         = AS_SLF.smass() / 1000
                    ...
                    # T_Line_Supply   = np.r_[IHX.T_out_h, Cycle.CoolingCoil.T_in_g]    # [point 9,5]
                    # s_Line_Supply   = np.r_[s_IHX_out, s_CC_in]
                    T_Line_Supply   = np.r_[IHX.T_out_h, Cycle.WavyChannel.T_in_g]    # [point 9,5]
                    s_Line_Supply   = np.r_[s_IHX[0], s_IHX[0]]
                    # line 5-6
                    # T_CC            = np.r_[Cycle.CoolingCoil.T_out_g, Cycle.CoolingCoil.T_in_g]    # [point 6,5]
                    # s_CC            = np.r_[s_CC_out, s_CC_in]
                    T_CC            = np.r_[Cycle.WavyChannel.T_out_g, Cycle.WavyChannel.T_in_g]    # [point 6,5]
                    s_CC            = np.r_[s_IHX[2], s_IHX[0]]
                    # line 6-8
                    # T_Line_Return   = np.r_[Cycle.CoolingCoil.T_out_g, IHX.T_in_h]    # [point 6,8]
                    # s_Line_Return   = np.r_[s_CC_out, s_IHX_in]
                    T_Line_Return   = np.r_[Cycle.WavyChannel.T_out_g, IHX.T_in_h]    # [point 6,8]
                    s_Line_Return   = np.r_[s_IHX[0], s_IHX[0]]
                    # line 9-8
                    # T_IHX_g_new   = np.r_[IHX.T_out_h, IHX.T_in_h]    # [point 9,8]
                    # s_IHX_g_new   = np.r_[s_IHX_out, s_IHX_in]
            ...
            # self.axes.plot(s_CC, T_CC, 'b-.')
            # self.axes.plot(s_Line_Return, T_Line_Return, 'b-.')
            print('T_8 = ', IHX.T_in_h, ' [K]; T_9 = ', IHX.T_out_h, ' [K]; T_5 = ', Cycle.WavyChannel.T_in_g, ' [K]; T_6 = ', Cycle.WavyChannel.T_out_g, '[K]')
    ...
    def PHOverlay(self, Cycle, **kwargs):
        ...
        # Critical isothermal
        self.axes.semilogy(h_lim, p_lim / 1000, 'y-.', lw=3, alpha=0.3)
