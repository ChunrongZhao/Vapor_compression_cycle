...
from ACHP_codes.Correlations.HTC_Correlations   import NaturalConv_HTC
...
class LiquidReceiverClass():
    ...
    """
    The liquid receiver acts as a damper in the cycle, absorbing the mass flow rate fluctuations.
    More concretely, a different explanation can be given.
    ...
    presence of liquid).
    In the inferior part of the tank, the mix of saturated and subcooled liquid (already present)
    allows the working fluid to exit it in a subcooled liquid state. The saturation pressure and
    temperature then reign then in the superior part of the reservoir.
    Thus, with this component, the charge fluctuations are literally absorbed, put to an equilibrium value,
    and the subcooling becomes null (this fact can't be stated in the presence of non-condensable gases).
    ...
    def OutputList(self):
        ...
        return [
            ...
            ('Inlet Pressure',                  'Pa',       self.p_in_r),
            ('Inlet Density',                   'kg/m3',    self.rho_in_r),
            ('Outlet Pressure',                 'Pa',       self.p_out_r)
           ]
    ...
    def Calculate(self):
        ...
        P1                          = self.p_in_r
        T1                          = self.T_in_r
        AS.update(CP.PT_INPUTS, self.p_in_r, self.T_in_r)
        self.rho_in_r               = AS.rhomass()              # [kg/m^3]
        self.s_in_r                 = AS.smass()                # [J/kg-K]
        self.h_in_r                 = AS.hmass()                # [J/kg]

        # Static pressure (rho*g*h) between inlet and outlet of the tank"
        self.p_out_r                = self.p_in_r               # + (rho_in*g*self.h_ports)/1000

        # Adiabatic
        self.Q                      = 0.0                       # [W]

        # No temperature gradient is observed in the reservoir.
        self.T_out_r                = self.T_in_r
        self.s_out_r                = self.s_in_r
        ...
        self.Charge                 = self.V_tank * self.rho_in_r
...
class SuctionAccumulatorClass():
    ...
    def Calculate(self):
        ...
        # Tank energy balance
        self.h_out_r        = self.h_in_r + self.Q

        # No temperature gradient is observed in the reservoir.
        AS.update(CP.HmassP_INPUTS, self.h_out_r, self.p_out_r)
        self.T_out_r        = AS.T()                                    # [K]
        self.rho_out_r      = AS.rhomass()                              # [kg/m3]
        self.s_out_r        = AS.smass()                                # [J/kg-K]

        # Use average density to calculate charge
        rho_tank            = (self.rho_in_r + self.rho_out_r) / 2
...
if __name__ == '__main__':
    ...
    print('T_in [K]',                   Accumulator.T_in_r)
    print('T_out [K]',                  Accumulator.T_out_r)
