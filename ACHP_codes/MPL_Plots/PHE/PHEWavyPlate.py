...
pylab.gca().add_patch(
    FancyArrowPatch((4 * np.pi, -1), (4 * np.pi, 1), arrowstyle='<|-|>', fc='k', ec='k', mutation_scale=20, lw=0.8))
pylab.text(4 * np.pi, 0, r' $2\hat a$', ha='left', va='center')
pylab.gca().add_patch(
    FancyArrowPatch((2 * np.pi, 1), (4 * np.pi, 1), arrowstyle='<|-|>', fc='k', ec='k', mutation_scale=20, lw=0.8))
